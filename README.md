...
1. Upload a new file to a GCS bucket folder
...
1. Add the ability to create a new folder in a GCS bucket
2. Add the ability to delete a folder from a GCS bucket
3. Add a new service for AWS (Amazon Web Services) S3 buckets (`service/aws.go`)
   with the same functionality as the GCS service (`service/gcs.go`)
4. Add a gRPC server (`server/grpc.go`) as an alternative to the current REST server
   (`server/rest.go`)
